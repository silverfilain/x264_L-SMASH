...
if [ $LOCAL_VER \> 1 ] ; then
    ...
    if [ $PLAIN_VER == $LOCAL_VER ] ; then
        ...
        git rev-list lsmash | sort > config.git-hash-lsmash
        LSMASH_VER=`git rev-list lorigin/lsmash | sort | join config.git-hash - | wc -l | awk '{print $1}'`
        LSMASH_BASE=`git rev-list porigin/master | sort | join config.git-hash-lsmash - | wc -l | awk '{print $1}'`
        rm -f config.git-hash-lsmash
        LSMASH_DIFF=$(($LSMASH_VER-$LSMASH_BASE))
        if [ $LSMASH_VER == $LOCAL_VER ] ; then
            VER_DIFF=$LSMASH_DIFF
        else
            if [ $LSMASH_BASE == $PLAIN_VER ] ; then
                LOCAL_DIFF=$(($LOCAL_VER-$PLAIN_VER-$LSMASH_DIFF))
            else
                LOCAL_DIFF=$(($LOCAL_VER-$PLAIN_VER-$LSMASH_DIFF-1))
            fi
            VER_DIFF=$LSMASH_DIFF+$LOCAL_DIFF
        fi
