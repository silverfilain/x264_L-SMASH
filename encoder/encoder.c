...
static int x264_validate_parameters( x264_t *h )
{
...

    {
        ...
        else
        {
            ...
            if( l->level_idc == 0 )
            {
                x264_log( h, X264_LOG_ERROR, "invalid level_idc: %d%s\n", h->param.i_level_idc, h->param.b_level_1b ? "(1b)" : "" );
                return -1;
            }
        }
...
x264_t *x264_encoder_open( x264_param_t *param )
{
    ...
    const char *level_1b = (h->sps->b_constraint_set3 || h->sps->i_level_idc < 10 ) ? "(1b)" : "";
