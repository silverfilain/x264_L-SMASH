...
void x264_sps_init( x264_sps_t *sps, int i_id, x264_param_t *param )
{
    ...
    sps->i_level_idc = ( param->i_level_idc == 11 && param->b_level_1b && sps->i_profile_idc >= PROFILE_HIGH ) ? 9 : param->i_level_idc;
    ...
    sps->b_constraint_set3  = ( sps->i_level_idc == 11 && param->b_level_1b );
...
static const x264_level_t x264_level_1b =
    { 11,  1485,    99,   152064,     128,    350,  64, 64,  0, 2, 0, 0, 1 };
static const x264_level_t x264_invalid_level = { 0 };
...
const x264_level_t* x264_get_level_constraints( x264_param_t *param )
{
    ...
    if( param->b_level_1b )
    {
        if( param->i_level_idc == 11 )
            return &x264_level_1b;
        else
            return &x264_invalid_level;
    }
...
const x264_level_t* x264_get_valid_level_constraints( x264_t *h )
{
    ...
    if( h->param.i_level_idc == 11 && !h->param.b_level_1b )
    {
        h->param.b_level_1b = 1;
        if( !x264_validate_levels( h, 0 ) )
            return &x264_level_1b;
        h->param.b_level_1b = 0;
    }
    return l;
}
