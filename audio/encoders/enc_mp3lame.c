...
#define XING_VBR_HEADER 0
#define SAMPLES_PER_GRANULE 576
#define DECDELAY 528 // + 0.5
...
static hnd_t init( hnd_t filter_chain, const char *opt_str )
{
    ...
    lame_set_bWriteVbrTag( h->lame, XING_VBR_HEADER );
    ...
    h->info.priming    = lame_get_encoder_delay( h->lame ) + DECDELAY + 1 + is_vbr * XING_VBR_HEADER * 1152;
...
static audio_packet_t *finish( hnd_t encoder )
{
    ...
    h->info.last_delta = h->info.priming + SAMPLES_PER_GRANULE - lame_get_encoder_padding( h->lame );
